...
export default function App() {
  ...
  const [settings, setSettings] = useState({ hfApiKey: "", s2ApiKey: "", crossrefEmail: "" });
  const [settingsSaved, setSettingsSaved] = useState(false);
  const [apiStatus, setApiStatus] = useState<{ s2: boolean | null; crossref: boolean | null }>({ s2: null, crossref: null });
  const inputRef = useRef<HTMLInputElement>(null);
  const reportRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    setSaved(lsGet("sc_saved") || []);
    setHistory(lsGet("sc_history") || []);
    setSettings(lsGet("sc_settings") || { hfApiKey: "", s2ApiKey: "", crossrefEmail: "" });
    checkApis();
  }, []);
  ...
  const runResearch = useCallback(async () => {
    const q = query.trim(); if (!q) { inputRef.current?.focus(); return; }
    ...
    try {
      const res = await fetch("/api/research", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ query: q }) });
      ...
      const entry: HistoryEntry = { id: Date.now().toString(), query: q, date: new Date().toLocaleDateString(), paperCount: newPapers.length, papers: newPapers, meta: newMeta };
  ...
  const navItems = [
    { id: "research" as Page, label: "Research", badge: null, icon: <svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="6" cy="6" r="4.5"/><path d="M9.5 9.5l3.5 3.5" strokeLinecap="round"/></svg> },
    { id: "library" as Page, label: "Library", badge: String(saved.length), icon: <svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M3 2h9v11H3z"/><path d="M6 5h3M6 7.5h3M6 10h2" strokeLinecap="round"/></svg> },
    { id: "reports" as Page, label: "Reports", badge: history.length > 0 ? String(history.length) : null, icon: <svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M2 13V2l5.5 2.5L13 2v11l-5.5-2.5L2 13z"/></svg> },
    { id: "settings" as Page, label: "Settings", badge: null, icon: <svg viewBox="0 0 15 15" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="7.5" cy="7.5" r="2"/><path d="M7.5 1v2M7.5 12v2M1 7.5h2M12 7.5h2M3.2 3.2l1.4 1.4M10.4 10.4l1.4 1.4M3.2 11.8l1.4-1.4M10.4 4.6l1.4-1.4" strokeLinecap="round"/></svg> },
  ];

  const pageTitles: Record<Page, string> = { research: "Research", library: "Library", reports: "Reports", settings: "Settings" };

  const Sidebar = () => (
    <aside style={{ background: "var(--bg-raised)", borderRight: "1px solid var(--border)", padding: "24px 18px", display: "flex", flexDirection: "column", gap: 28, position: "sticky", top: 0, height: "100vh", overflowY: "auto", width: 252 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 32, height: 32, borderRadius: "var(--r-sm)", background: "var(--bg-card)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="17" height="17" viewBox="0 0 17 17" fill="none" stroke="var(--accent)" strokeWidth="1.5"><circle cx="7" cy="7" r="5"/><path d="M11 11l4 4" strokeLinecap="round"/><path d="M7 4.5v5M4.5 7h5" strokeLinecap="round"/></svg>
        </div>
        <div>
      ...
      <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: 2 }}>
        {navItems.map(item => (
          <li key={item.id} onClick={() => { setPage(item.id); setSidebarOpen(false); }}
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: "var(--r-sm)", fontSize: 13, fontWeight: 500, color: page === item.id ? "var(--accent)" : "var(--text-dim)", background: page === item.id ? "rgba(94,230,201,0.08)" : "transparent", cursor: "pointer", transition: "all 0.15s", position: "relative" }}>
            ...
            {item.label}
            {item.badge !== null && <span style={{ marginLeft: "auto", fontFamily: "var(--mono)", fontSize: 10, background: "var(--bg-card)", color: "var(--text-faint)", padding: "1px 6px", borderRadius: 8, border: "1px solid var(--border)" }}>{item.badge}</span>}
          </li>
        ))}
      </ul>
      <div style={{ marginTop: "auto", borderTop: "1px solid var(--border)", paddingTop: 14, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ fontSize: 12 }}><div style={{ fontWeight: 600 }}>Researcher</div><div style={{ color: "var(--text-faint)", fontSize: 10.5 }}>Free plan</div></div>
      </div>
    </aside>
  );
  ...
  const LibraryPage = () => (
    <div style={{ padding: "28px 32px" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--text-faint)" }}>{saved.length} saved paper{saved.length !== 1 ? "s" : ""} · localStorage mein saved</div>
      </div>
      {saved.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", background: "var(--bg-raised)", borderRadius: "var(--r-lg)", border: "1px solid var(--border)", color: "var(--text-faint)" }}>
          ...
          <div style={{ fontSize: 13 }}>Research karo → papers pe "+ Save" click karo<br/>Refresh ke baad bhi yahan milenge!</div>
        </div>
      ) : (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-faint)", textTransform: "uppercase" }}>{saved.length} Papers</span>
            <button onClick={() => { setSaved([]); lsSet("sc_saved", []); }} style={{ fontSize: 11, padding: "5px 10px", borderRadius: 6, border: "1px solid var(--rose)", background: "transparent", color: "var(--rose)", cursor: "pointer" }}>Clear All</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {saved.map(paper => (
              <div key={paper.id} style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
                <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 6 }}>{paper.url ? <a href={paper.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--text)", textDecoration: "none" }}>{paper.title}</a> : paper.title}</div>
                <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-faint)", marginBottom: 8, display: "flex", gap: 12, flexWrap: "wrap" }}>
                  <span>✎ {paper.authors}</span><span>◷ {paper.year}</span><span>⊞ {paper.journal}</span><span>❝ {paper.cites.toLocaleString()}</span>
                </div>
  ...
  const ReportsPage = () => (
    <div style={{ padding: "28px 32px" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--text-faint)" }}>Search history · {history.length} session{history.length !== 1 ? "s" : ""}</div>
      </div>
      {history.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", background: "var(--bg-raised)", borderRadius: "var(--r-lg)", border: "1px solid var(--border)", color: "var(--text-faint)" }}>
          <div style={{ fontSize: 36, marginBottom: 12, opacity: 0.3 }}>📋</div>
          <div style={{ fontFamily: "var(--display)", fontSize: 18, color: "var(--text-dim)", marginBottom: 8 }}>Koi Report Nahi</div>
          <div style={{ fontSize: 13 }}>Research karo — history yahan automatically save hogi!</div>
        </div>
      ) : (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-faint)", textTransform: "uppercase" }}>{history.length} Searches</span>
            <button onClick={() => { setHistory([]); lsSet("sc_history", []); }} style={{ fontSize: 11, padding: "5px 10px", borderRadius: 6, border: "1px solid var(--rose)", background: "transparent", color: "var(--rose)", cursor: "pointer" }}>Clear History</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {history.map(entry => (
              <div key={entry.id} style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                  <div>
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-faint)", display: "flex", gap: 12 }}>
                      <span>📅 {entry.date}</span><span>📄 {entry.paperCount} papers</span><span>⏱ {entry.meta.latency}</span>
                    </div>
                  </div>
    ...
    <div style={{ padding: "28px 32px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 520 }}>
        <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
          ...
          {[{ name: "Semantic Scholar", status: apiStatus.s2 }, { name: "CrossRef", status: apiStatus.crossref }].map(({ name, status }) => (
            <div key={name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid var(--border-soft)" }}>
              ...
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, padding: "3px 10px", borderRadius: 12, background: status === null ? "var(--bg-card)" : status ? "rgba(94,230,201,0.1)" : "rgba(229,112,122,0.1)", color: status === null ? "var(--amber)" : status ? "var(--accent)" : "var(--rose)", border: `1px solid ${status === null ? "var(--border)" : status ? "var(--accent-dim)" : "var(--rose)"}` }}>
                {status === null ? "Checking…" : status ? "● Online" : "✕ Offline"}
              </span>
            </div>
          ))}
          <button onClick={checkApis} style={{ marginTop: 12, fontSize: 11, padding: "6px 12px", borderRadius: 6, border: "1px solid var(--border)", background: "transparent", color: "var(--text-dim)", cursor: "pointer" }}>🔄 Refresh Status</button>
        </div>
        {[
          { key: "hfApiKey", label: "Hugging Face API Key", placeholder: "hf_xxxxxxxxxxxxxxxx", type: "password" },
          { key: "s2ApiKey", label: "Semantic Scholar API Key", placeholder: "xxxxxxxxxxxxxxxx", type: "password" },
          { key: "crossrefEmail", label: "CrossRef Email", placeholder: "your@email.com", type: "email" },
        ].map(({ key, label, placeholder, type }) => (
          <div key={key} style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
            <label style={{ display: "block", fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", color: "var(--text-faint)", marginBottom: 8 }}>{label}</label>
        ...
        <button onClick={() => { lsSet("sc_settings", settings); setSettingsSaved(true); setTimeout(() => setSettingsSaved(false), 2500); }}
          style={{ fontFamily: "var(--sans)", fontSize: 13, fontWeight: 600, padding: "11px 20px", borderRadius: "var(--r-sm)", border: "none", background: settingsSaved ? "var(--accent-dim)" : "var(--accent)", color: "#06231c", cursor: "pointer" }}>
  ...
  const ResearchPage = () => (
    <div style={{ padding: "28px 32px 48px", flex: 1 }}>
      <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "24px 26px", marginBottom: 24, position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "-40%", right: "-10%", width: 280, height: 280, background: "radial-gradient(circle, rgba(94,230,201,0.06), transparent 70%)", pointerEvents: "none" }} />
        <div style={{ fontFamily: "var(--mono)", fontSize: 10.5, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
          <svg width="12" height="12" viewBox="0 0 12 12" fill="var(--accent)"><circle cx="6" cy="6" r="3"/></svg>
          Multi-Agent Research Pipeline
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 200, position: "relative", display: "flex", alignItems: "center" }}>
            <svg style={{ position: "absolute", left: 14, width: 16, height: 16, color: "var(--text-faint)" }} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="7" cy="7" r="5"/><path d="M11 11l3.5 3.5" strokeLinecap="round"/></svg>
            <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && runResearch()}
              placeholder="e.g. machine learning in drug discovery"
              style={{ width: "100%", fontFamily: "var(--sans)", fontSize: 14, padding: "12px 14px 12px 40px", borderRadius: "var(--r-sm)", border: "1px solid var(--border)", background: "var(--bg-card)", color: "var(--text)", outline: "none" }}
              onFocus={e => (e.target.style.borderColor = "var(--accent)")}
              onBlur={e => (e.target.style.borderColor = "var(--border)")}
            />
          </div>
          <button onClick={runResearch} disabled={loading}
            ...
            {loading ? <><span style={{ width: 14, height: 14, border: "2px solid #06231c", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite", display: "inline-block" }}/>Running…</> : "Run Research ↵"}
          </button>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
          {["climate change adaptation", "CRISPR gene editing", "transformer neural networks", "antibiotic resistance"].map(t => (
            <button key={t} onClick={() => setQuery(t)} style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-dim)", display: "flex", alignItems: "center", gap: 6, background: "var(--bg-card)", padding: "5px 10px", borderRadius: 6, border: "1px solid var(--border)", cursor: "pointer" }}>
      ...
      {error && <div style={{ background: "rgba(229,112,122,0.1)", border: "1px solid var(--rose)", borderRadius: "var(--r-md)", padding: "14px 18px", marginBottom: 20, color: "var(--rose)", fontSize: 13 }}>⚠ {error}</div>}
      ...
      {pStatus !== "idle" && (
        <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "22px 26px", marginBottom: 24 }}>
          <div style={{ fontFamily: "var(--mono)", fontSize: 11, textTransform: "uppercase", color: "var(--text-faint)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
            Agent Pipeline <span style={{ flex: 1, height: 1, background: "var(--border)" }}/> <span style={{ color: pStatus === "done" ? "var(--accent)" : "var(--amber)" }}>{pStatus === "done" ? "Complete" : "Running"}</span>
          </div>
      ...
      {papers.length > 0 && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 308px", gap: 20, alignItems: "start" }}>
            <div>
              <div style={{ fontFamily: "var(--mono)", fontSize: 11, textTransform: "uppercase", color: "var(--text-faint)", marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}>
                Results <span style={{ color: "var(--accent)" }}>{papers.length} papers</span><span style={{ flex: 1, height: 1, background: "var(--border)" }}/>
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {papers.map(paper => (
                  <div key={paper.id} style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 7 }}>
                      <div style={{ fontWeight: 600, fontSize: 14.5, lineHeight: 1.4 }}>{paper.url ? <a href={paper.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--text)", textDecoration: "none" }}>{paper.title}</a> : paper.title}</div>
                      <span style={{ fontFamily: "var(--mono)", fontSize: 10.5, fontWeight: 600, color: "var(--accent)", background: "rgba(94,230,201,0.08)", padding: "3px 9px", borderRadius: 6, flexShrink: 0, border: "1px solid var(--accent-dim)" }}>{paper.match}% match</span>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 7 }}>
                      ...
                      {paper.url && <a href={paper.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, fontWeight: 600, padding: "6px 11px", borderRadius: 6, border: "1px solid var(--border)", background: "transparent", color: "var(--text-faint)", textDecoration: "none" }}>↗ Open Paper</a>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div>
              <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px", marginBottom: 16 }}>
                <div style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", color: "var(--text-faint)", marginBottom: 13 }}>Run Metrics</div>
                {meta && [["Papers scanned", meta.scanned.toLocaleString(), "accent"], ["Matched", String(meta.total), ""], ["Latency", meta.latency, "violet"], ["HF AI", meta.hfEnabled ? "Active" : "Off", ""]].map(([l, v, c]) => (
                  <div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--border-soft)" }}>
                    <span style={{ fontSize: 12, color: "var(--text-dim)" }}>{l}</span>
                    <span style={{ fontFamily: "var(--mono)", fontSize: 12, fontWeight: 600, color: c === "accent" ? "var(--accent)" : c === "violet" ? "var(--violet)" : "var(--text)" }}>{v}</span>
                  </div>
                ))}
              </div>
              <div style={{ background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-md)", padding: "17px 19px" }}>
                <div style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", color: "var(--text-faint)", marginBottom: 13 }}>Export</div>
                <button onClick={() => downloadReport(papers, meta)} style={{ width: "100%", fontSize: 12, fontWeight: 600, padding: "9px 14px", borderRadius: "var(--r-sm)", border: "1px solid var(--border)", background: "var(--bg-card)", color: "var(--text-dim)", cursor: "pointer" }}>↓ Download Report</button>
              </div>
            </div>
          </div>

          <div ref={reportRef} style={{ marginTop: 32, background: "var(--bg-raised)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", padding: "28px 32px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              {papers.map((p, i) => (
                <div key={p.id} style={{ paddingLeft: 20, borderLeft: "2px solid var(--border)", position: "relative" }}>
                  ...
                  <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text-faint)", marginBottom: 6 }}>{p.authors} · {p.year} · <em>{p.journal}</em> · {p.cites.toLocaleString()} citations</div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      {/* Mobile top bar */}
      <div style={{ display: "none", position: "fixed", top: 0, left: 0, right: 0, zIndex: 20, background: "var(--bg-raised)", borderBottom: "1px solid var(--border)", padding: "14px 18px", alignItems: "center", justifyContent: "space-between" }}
      ...
      {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 25 }}/>}
      <div className={`sidebar-mobile ${sidebarOpen ? "sidebar-open" : ""}`}><Sidebar /></div>
      ...
      <main style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 32px", borderBottom: "1px solid var(--border)", background: "rgba(10,14,20,0.7)", backdropFilter: "blur(6px)", position: "sticky", top: 0, zIndex: 10 }}>
          <div>
            <div style={{ fontFamily: "var(--display)", fontSize: 20, fontWeight: 700 }}>{pageTitles[page]}</div>
